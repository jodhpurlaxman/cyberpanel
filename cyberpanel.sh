...
special_change() {
  ...
  sed -i 's|git clone https://github.com/jodhpurlaxman/cyberpanel|echo downloaded|g' install.py
...
EOF
  ...
  if [[ $SERVER_COUNTRY == "CN" ]]; then
    #line1="$(grep -n "github.com/jodhpurlaxman/cyberpanel" install.py | head -n 1 | cut -d: -f1)"
    #line2=$((line1 - 1))
    ...
    sed -i 's|git clone https://github.com/jodhpurlaxman/cyberpanel|wget https://cyberpanel.sh/cyberpanel-git.tar.gz \&\& tar xzvf cyberpanel-git.tar.gz|g' install.py
...
pip_virtualenv() {
  ...
  if [[ $DEV == "ON" ]]; then
    #install dev branch
    wget https://raw.githubusercontent.com/jodhpurlaxman/cyberpanel/$BRANCH_NAME/requirments.txt -O /usr/local/requirments.txt
