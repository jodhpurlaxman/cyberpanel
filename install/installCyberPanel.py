...
class InstallCyberPanel:
    ...
    def changeMYSQLRootPassword(self):
        if self.remotemysql == 'OFF':
            ...
            if self.distro == ubuntu:
                passwordCMD = "SET GLOBAL innodb_default_row_format = 'DYNAMIC';use mysql;DROP DATABASE IF EXISTS test;'DELETE FROM mysql.db WHERE Db='test' OR Db='test\\_%%';GRANT ALL PRIVILEGES ON *.* TO 'root'@'localhost' IDENTIFIED BY '%s';UPDATE user SET plugin='' WHERE User='root';flush privileges;" % (InstallCyberPanel.mysql_Root_password)
            else:
                passwordCMD = "SET GLOBAL innodb_default_row_format = 'DYNAMIC';use mysql;DROP DATABASE IF EXISTS test;DELETE FROM mysql.db WHERE Db='test' OR Db='test\\_%%';GRANT ALL PRIVILEGES ON *.* TO 'root'@'localhost' IDENTIFIED BY '%s';flush privileges;" % (InstallCyberPanel.mysql_Root_password)
